...
export const TextPanel = ({ setPanel }: { setPanel: (p: string | null) => void }) => {
  const { canvas, customVariables, addCustomVariable } = useDesignerStore();
  ...
  const variableGroups = [
    {
      ...
      icon: <Type size={14} />,
      items: [
        { label: 'Full Name', ph: '{{full_name}}' },
        { label: 'First Name', ph: '{{first_name}}' },
        { label: 'Last Name', ph: '{{last_name}}' },
        { label: 'ID Number', ph: '{{id_number}}' },
        { label: 'Employee ID', ph: '{{employee_id}}' },
        { label: 'Student ID', ph: '{{student_id}}' },
        { label: 'Username', ph: '{{username}}' },
      ]
    },
    {
      ...
      icon: <Plus size={14} />,
      items: [
        { label: 'DOB', ph: '{{date_of_birth}}' },
        { label: 'Gender', ph: '{{gender}}' },
        { label: 'Blood Group', ph: '{{blood_group}}' },
        { label: 'Nationality', ph: '{{nationality}}' },
        { label: 'Marital Status', ph: '{{marital_status}}' },
      ]
    },
    {
      ...
      icon: <Type size={14} />,
      items: [
        { label: 'Phone', ph: '{{phone_number}}' },
        { label: 'Alt Phone', ph: '{{alternate_phone}}' },
        { label: 'Email', ph: '{{email}}' },
        { label: 'Address', ph: '{{address}}' },
        { label: 'City', ph: '{{city}}' },
        { label: 'State', ph: '{{state}}' },
        { label: 'Pincode', ph: '{{pincode}}' },
      ]
    },
    {
      ...
      items: [
        { label: 'Company', ph: '{{company_name}}' },
        { label: 'Comp. Address', ph: '{{company_address}}' },
        { label: 'Comp. Phone', ph: '{{company_phone}}' },
        { label: 'Comp. Email', ph: '{{company_email}}' },
        { label: 'Website', ph: '{{website}}' },
      ]
    },
    {
      name: 'Job Details',
      icon: <CreditCard size={14} />,
      items: [
        { label: 'Designation', ph: '{{designation}}' },
        { label: 'Department', ph: '{{department}}' },
        { label: 'Emp. Type', ph: '{{employee_type}}' },
        { label: 'Joining Date', ph: '{{joining_date}}' },
        { label: 'Location', ph: '{{work_location}}' },
        { label: 'Shift', ph: '{{shift}}' },
      ]
    },
    {
      ...
      icon: <CreditCard size={14} />,
      items: [
        { label: 'Class', ph: '{{class}}' },
        { label: 'Section', ph: '{{section}}' },
        { label: 'Roll No', ph: '{{roll_number}}' },
        { label: 'Course', ph: '{{course}}' },
        { label: 'Year', ph: '{{year}}' },
      ]
    },
    {
      ...
      items: [
        { label: 'Issue Date', ph: '{{issue_date}}' },
        { label: 'Expiry Date', ph: '{{expiry_date}}' },
        { label: 'Valid Till', ph: '{{valid_till}}' },
        { label: 'Status', ph: '{{card_status}}' },
      ]
    },
    {
      ...
      icon: <ShieldCheck size={14} />,
      items: [
        { label: 'Emerg. Contact', ph: '{{emergency_contact_name}}' },
        { label: 'Emerg. No', ph: '{{emergency_contact_number}}' },
        { label: 'Medical Notes', ph: '{{medical_notes}}' },
        { label: 'Serial No', ph: '{{serial_number}}' },
        { label: 'Batch ID', ph: '{{batch_id}}' },
      ]
    }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="p-4 bg-green-50/50 rounded-2xl border border-green-100/50">
        <label className="text-[11px] font-bold text-green-700 block mb-3 uppercase tracking-wider">Create Custom Variable</label>
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 border border-green-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-green-500 bg-white shadow-inner"
          />
          <button
            onClick={() => {
              if (newVar.trim()) {
                addCustomVariable(newVar.trim());
                setNewVar('');
              }
            }}
            className="px-4 py-2 bg-green-600 text-white rounded-xl text-xs font-bold hover:bg-green-700 transition-colors shadow-sm"
          >
            Save
          </button>
        </div>
        ...
        {customVariables.length > 0 && (
          <section>
            <div className="flex items-center gap-2 mb-3">
              ...
              <label className="text-xs font-bold text-green-700 uppercase tracking-wider">Custom Saved</label>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {customVariables.map(v => (
                <button
                  key={v}
                  onClick={() => addText(`{{${v.toLowerCase().replace(/\s+/g, '_')}}}`, true)}
                  className="flex flex-col items-center justify-center p-5 border border-green-100 bg-green-50/30 rounded-3xl hover:border-green-400 hover:shadow-xl hover:scale-[1.02] transition-all group active:scale-95 shadow-sm"
                >
                  <div className="w-12 h-12 rounded-2xl bg-green-100/50 flex items-center justify-center mb-2 group-hover:bg-green-100 transition-colors">
                    <Type size={18} className="text-green-600" />
                  </div>
                  <span className="text-[11px] font-black text-green-700 text-center leading-tight">{v}</span>
                </button>
              ))}
            </div>
          </section>
      ...
      <div className="space-y-6">
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 h-12 rounded-xl font-bold transition-all active:scale-95 shadow-lg ${
              isProcessingBG 
            ...
            <div className="flex border border-gray-200 rounded-lg divide-x divide-gray-200 overflow-hidden">
              {['left', 'center', 'right'].map(align => (
                <button
                  ...
                  className={`p-2 hover:bg-gray-50 ${props.textAlign === align ? 'bg-green-50 text-green-600' : ''}`}
                >
                  <SlidersHorizontal size={14} className={align === 'center' ? 'rotate-90' : ''} />
                </button>
              ))}
            </div>
          </div>
...
const addImageToCanvas = async (ph: string, label: string, canvas: fabric.Canvas | null, setActivePanel?: (p: string | null) => void) => {
  ...
  try {
    ...
    console.error('Error generating initial security image:', err);
  }
