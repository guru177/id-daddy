...
const DesignCard = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex flex-col group text-left w-full bg-white rounded-[28px] border border-stone-200/60 overflow-hidden transition-all duration-500 ${hoverShadow} ${hoverBorder} group-hover:-translate-y-2`}
    >
      {/* Dual-side preview — dynamic aspect ratio to see full design */}
      <div className={`relative w-full ${isHorizontal ? 'aspect-[86/108]' : 'aspect-[108/86]'} bg-stone-50 overflow-hidden group-hover:bg-green-50/20 transition-colors duration-500`}>
      ...
      <div className="px-4 py-2 border-t border-gray-50">
        <h3 className={`text-xs font-black text-gray-900 truncate transition-colors ${accentColor === 'green' ? 'group-hover:text-green-600' : 'group-hover:text-amber-600'}`}>
          {design.name}
        </h3>
        <p className="text-[9px] font-bold text-gray-900 uppercase tracking-widest mt-0.5">
        ...
        <section className="mb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            ...
            <button
              ...
              className="flex flex-col group w-full text-left bg-white rounded-[28px] border-2 border-dashed border-green-100/60 overflow-hidden transition-all duration-500 hover:border-green-400  hover:-translate-y-2"
            >
              <div className="relative w-full aspect-[108/86] flex flex-col items-center justify-center gap-4 bg-stone-50/30 group-hover:bg-green-50/20 transition-colors duration-500">
                <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center text-green-200  transition-all duration-500 group-hover:text-green-600  group-hover:rotate-90">
                  <Plus size={28} strokeWidth={2.5} />
                </div>
              </div>
              <div className="px-4 py-2 border-t border-gray-50">
                <h3 className="text-xs font-black text-gray-900 group-hover:text-green-600 transition-colors">Blank Canvas</h3>
                <p className="text-[9px] font-bold text-gray-900 uppercase tracking-widest mt-0.5">Start from scratch</p>
              </div>
            </button>
