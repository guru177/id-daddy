...
export function DashboardView() {
  ...
  const organizationType = useDesignerStore((state) => state.organizationType);
  ...
  const deptLabel = useMemo(() => organizationType === 'education' ? 'Class' : 'Department', [organizationType]);
  const deptLabelPlural = useMemo(() => organizationType === 'education' ? 'Classes' : 'Departments', [organizationType]);
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#a8a29e', fontSize: 12, fontWeight: 'bold' }} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#a8a29e', fontSize: 12, fontWeight: 'bold' }} />
                  <Tooltip 
                    cursor={{ fill: '#fdfaf5', radius: 12 }} 
                    contentStyle={{ borderRadius: '20px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)', padding: '12px' }} 
                  />
                  <Bar dataKey="count" fill="url(#greenGradient)" radius={[10, 10, 0, 0]} maxBarSize={60}>
                    {chartData.map((_entry, index) => (
                      <Cell key={`cell-${index}`} fill={index === 0 ? '#1a5d1a' : '#2d7a2d'} opacity={1 - (index * 0.1)} />
                    ))}
                  </Bar>
                  <defs>
                    <linearGradient id="greenGradient" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#1a5d1a" />
              ...
              {members.slice(0, 5).map(m => (
                <div key={m.id} className="flex items-center gap-3 lg:gap-4 bg-[#fdfaf5] hover:bg-white hover:shadow-xl hover:scale-[1.02] border border-stone-100 rounded-[24px] lg:rounded-[28px] p-3 transition-all group">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm lg:text-base font-black text-stone-900 truncate tracking-tight">{m.firstName} {m.lastName}</p>
                    <p className="text-[10px] font-bold text-stone-900 truncate tracking-widest uppercase mt-0.5">{m.idNumber || m.employeeId || 'No ID'}</p>
                  </div>
                </div>
              ))}
              {members.length === 0 && (
...
function Metric({ label, value, icon: Icon }: { label: string; value: number; icon: typeof LayoutTemplate }) {
  return (
    <div className="bg-white p-5 lg:p-6 rounded-[28px] lg:rounded-[32px] shadow-2xl shadow-stone-200/50 border border-white flex items-center gap-4 lg:gap-5 hover:scale-[1.02] transition-all group">
      ...
      <div>
        <p className="text-[10px] lg:text-[11px] font-black text-stone-900 uppercase tracking-widest mb-0.5">{label}</p>
        <p className="text-3xl lg:text-4xl font-black text-stone-900 tracking-tighter leading-none">{value}</p>
      </div>
    </div>
  );
}
...
function QuickLink({ icon: Icon, label, desc, onClick }: { icon: typeof LayoutTemplate; label: string; desc: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="bg-white p-4 lg:p-6 rounded-[28px] lg:rounded-[32px] shadow-2xl shadow-stone-200/50 border border-white hover:scale-[1.05] hover:shadow-green-900/10 transition-all text-left group">
      ...
      <p className="text-[11px] lg:text-xs font-bold text-stone-900 leading-relaxed">{desc}</p>
    </button>
  );
}
